...
@pytest.mark.asyncio
async def test_market_fresh_imported_fallback_refreshes_after_source_row_commit(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    plugin_id = "market_imported_refresh"
    package_source_root = tmp_path / "package-source"
    package_source_root.mkdir()
    package_path = package_source_root / f"{plugin_id}.neko-plugin"
    pack_plugin(_make_plugin_dir(tmp_path / "source", plugin_id=plugin_id), package_path)
    ...
    try:
        result = await PluginCliService().upload_and_install(
            ...
            install_source_override={
                "channel": "market",
                "mode": "install",
                "market_detail": {"plugin_market_id": plugin_id},
            },
        )
    finally:
        set_global_manager(None)
    ...
    assert refresh_calls == [(plugin_id, True)]
    assert result["install"]["channel"] == "imported"
    assert "falling back to imported channel" in str(result["install_source_warning"])
